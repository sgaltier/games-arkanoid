...
**Status:** 55 findings fixed. See [todo.md](todo.md) for what's still open.
...
> [578–579](../html/index.html#L578-L579), [3494–3495](../html/index.html#L3494-L3495).
...
> ([1448–1462](../html/index.html#L1448-L1462)), both wrapped in `try/catch`, with the best score
> degrading to in-memory only. Call sites: [1559](../html/index.html#L1559),
> [2953](../html/index.html#L2953).
...
> [1865–1871](../html/index.html#L1865-L1871).

`keydown` sets `state.keys[e.code] = true` [1823](../html/index.html#L1823) and only `keyup` cleared it
[1864](../html/index.html#L1864). Alt-tabbing (or hitting a browser shortcut) while holding <kbd>→</kbd>
...
> feeds while the phase is `playing`. See [2509–2520](../html/index.html#L2509-L2520), the effect
...
> **Fixed 2026-08-12.** `autoPause()` ([1873–1881](../html/index.html#L1873-L1881)) pauses whenever the
> phase is `playing`, wired to both `visibilitychange` and the existing `blur` handler from #3
> ([1868–1871](../html/index.html#L1868-L1871)). It deliberately only fires *on* hide/blur, never on
...
`dt` is clamped to 33 ms [3423](../html/index.html#L3423), so the game didn't *jump* — but it stayed in
...
> `isTypingTarget()` and widened to cover text inputs too by #42, [1808–1812](../html/index.html#L1808-L1812),
> used at [1854](../html/index.html#L1854)): when a `<button>` holds focus the key is handed back to the
...
> `blurIfPointerClick` helper ([2240–2245](../html/index.html#L2240-L2245)) drops focus after pointer
...
> alongside the existing pointer-release logic — [1846–1853](../html/index.html#L1846-L1853). Applied to
...
> `e.button === 0` — [1888–1891](../html/index.html#L1888-L1891).
...
> [2812](../html/index.html#L2812). A paddle collision is only resolved as a top-face bounce — steering
> by offset and snapping onto the top — when `prevY` was already above the paddle top
> [2855–2863](../html/index.html#L2855-L2863); otherwise it resolves as a side hit that reflects only
> the horizontal component and repositions the ball beside the paddle, the same treatment a brick's
> side face gets [2865–2873](../html/index.html#L2865-L2873). Paddle-velocity spin was left for a
...
> [2776–2785](../html/index.html#L2776-L2785) — the smaller of the two axis overlaps, i.e. how shallow
> the intrusion is — and resolves against whichever brick has the smallest penetration
> [2876–2894](../html/index.html#L2876-L2894). Array order no longer has any say in which face gets hit.
...
> capsule with — [2608–2609](../html/index.html#L2608-L2609) vs. the `arc(0, 0, 10, …)` at
> [3346](../html/index.html#L3346).
...
> it's descending, then spread symmetrically to either side — [2584–2588](../html/index.html#L2584-L2588).
...
> shared `maybeSaveBest()` helper [2943–2951](../html/index.html#L2943-L2951), called from
> `checkLevelClear()` [2959](../html/index.html#L2959) as well as `endGame()`
> [2975](../html/index.html#L2975). Progress is now checkpointed at every level clear, not just at the
> end of the run.
...
> [1150](../html/index.html#L1150); `drawDrops` just assigns it — [3349](../html/index.html#L3349). The
...
> **Fixed 2026-08-13.** A `hudLast` cache [3098](../html/index.html#L3098) records what's currently
> displayed for each of the four HUD fields; `updateHud()` [3099–3110](../html/index.html#L3099-L3110)
> only touches `textContent` for a field whose value actually changed since the last call. The
> unconditional per-frame call [3465](../html/index.html#L3465) stays — it's still what catches
...
> **Fixed 2026-08-13.** `state.remainingBricks` [1576](../html/index.html#L1576) counts destructible
> bricks still alive; `buildLevel()` seeds it when a level starts
> [1642](../html/index.html#L1642)/[1663](../html/index.html#L1663), and `brickHit()` decrements it at the
> single point a brick actually dies [2734](../html/index.html#L2734). `checkLevelClear()`
> [2957–2966](../html/index.html#L2957-L2966) is now an `O(1)` counter check instead of an `O(n)` scan.
...
> **Fixed 2026-08-13.** `setPhase()` [2117](../html/index.html#L2117) now owns every phase→overlay
> mapping via a `PHASE_OVERLAY` lookup [2047–2061](../html/index.html#L2047-L2061), extended to cover
> `levelclear`/`victory`/`gameover` as well as the phases it already handled. `togglePause`
> [1972](../html/index.html#L1972), `checkLevelClear` [2969](../html/index.html#L2969), and `endGame`
> [2517](../html/index.html#L2517) now all just call `setPhase(...)` instead of duplicating the
...
`setPhase` [2117](../html/index.html#L2117) was the intended single entry point, but `togglePause`,
...
>   thing ~16 ms later via `draw()` [3401–3417](../html/index.html#L3401-L3417), and the HUD's own
>   one-time init call [3111](../html/index.html#L3111) already covers the pre-play text.
> - `updateBalls` [2800](../html/index.html#L2800) now declares only the `dt` parameter it uses; the
>   call site [3459](../html/index.html#L3459) no longer passes the unused `now`.
...
> **Fixed 2026-08-13.** `audioCtx()` [2295](../html/index.html#L2295) — `beep()`'s own body when
> this was written, split out by #59 — now calls `actx.resume()`
> [2304](../html/index.html#L2304) whenever the context is `"suspended"` — cheap and a no-op once
...
> `loadMuted()`/`saveMuted()` [1470–1471](../html/index.html#L1470-L1471), the same `storageGet`/
> `storageSet` pair [1448–1462](../html/index.html#L1448-L1462) already used for the best score and the
> language preference, written on every toggle [2273](../html/index.html#L2273) and read back into
> `state.muted` at boot [1580](../html/index.html#L1580).
...
> **Fixed 2026-08-13.** A single `CONFIG` object [1169–1264](../html/index.html#L1169-L1264) now holds drop
...
> [2604](../html/index.html#L2604), particle gravity [2648](../html/index.html#L2648), the ball cap in
> both of `applyPowerup`'s multi-ball checks [2579](../html/index.html#L2579)/
> [2586](../html/index.html#L2586), the paddle bounce spread [2859](../html/index.html#L2859), and the
> four original effect branches [2555–2566](../html/index.html#L2555-L2566).
...
> one shown at boot. `showOverlay()` [2080–2112](../html/index.html#L2080-L2112) keeps `aria-hidden` in
...
> sync by their render functions. `renderMuteButton()` [2162–2167](../html/index.html#L2162-L2167) now
> also sets `aria-pressed`; a new `renderPauseButton()`
> [2173–2179](../html/index.html#L2173-L2179) mirrors it for pause, and — since the pause button used to
...
> from both `setPhase()` [2120](../html/index.html#L2120) and `applyLanguage()`
> [2216](../html/index.html#L2216), so it stays correct across phase changes and language switches
...
> ([2162–2167](../html/index.html#L2162-L2167)) sets the mute button's `aria-label` from both the
...
> **Fixed 2026-08-13.** `burst()` [1726](../html/index.html#L1726) now scales its particle count down to
> roughly a third (never below 1) whenever `reduceMotion` is true, read from
> `matchMedia("(prefers-reduced-motion: reduce)")` [1719–1723](../html/index.html#L1719-L1723) — live,
...
> **Fixed 2026-08-13.** `showOverlay()` [2080–2112](../html/index.html#L2080-L2112) now focuses the
...
`handleLaunchOrResume` [1925](../html/index.html#L1925) only handled `ready` and `paused`. From
...
> [1908–1923](../html/index.html#L1908-L1923); `touchstart`/`touchmove`
> [1896–1907](../html/index.html#L1896-L1907) now only update `pointerX`, aiming the paddle. That gives
...
`touchstart` [1896](../html/index.html#L1896) (previously) set `pointerX` and immediately called
...
> **Fixed 2026-08-13.** `state.difficultyMult` [1578](../html/index.html#L1578) multiplies directly into
> ball velocity [2813](../html/index.html#L2813), alongside the existing power-up speed multiplier. It
> ramps via `bumpDifficulty()` [1632–1634](../html/index.html#L1632-L1634) — cumulative, multiplicative,
> capped at `CONFIG.difficulty.max` — from two classic-Breakout triggers: every top-wall bounce
> [2819–2825](../html/index.html#L2819-L2825), and every `CONFIG.difficulty.brickMilestone` bricks
> destroyed in the current level [2735–2738](../html/index.html#L2735-L2738). `CONFIG.difficulty`
> [1208–1213](../html/index.html#L1208-L1213) holds the tuning; `buildLevel()`
> [1664–1665](../html/index.html#L1664-L1665) resets both the multiplier and the milestone counter at the
> start of every level, so the ramp never carries over from one level — or one difficulty — to the
> next.

Ball speed was fixed per level ([1384](../html/index.html#L1384), `LEVELS[i].speed`). Classic breakout
...
> ([1741–1746](../html/index.html#L1741-L1746), rising and fading over `CONFIG.floatingText.life`
> seconds via `updateFloatingTexts()`/`drawFloatingTexts()`
> [2652–2659](../html/index.html#L2652-L2659)/[3384–3399](../html/index.html#L3384-L3399)), wired into
> the frame loop alongside particles [3463](../html/index.html#L3463)/[3468](../html/index.html#L3468)
> and `draw()` [3415](../html/index.html#L3415). Consecutive bricks destroyed without the ball touching
> the paddle also build a combo [2742–2747](../html/index.html#L2742-L2747) that scales the points
> awarded, capped at `CONFIG.combo.max`; any paddle contact — top face or side clip — resets it
> [2843](../html/index.html#L2843). `CONFIG.combo`/`CONFIG.floatingText`
> [1214–1276](../html/index.html#L1214-L1276) hold the tuning. This changes the scoring curve going forward
...
> `applyPowerup` [2570–2575](../html/index.html#L2570-L2575).
>
> **Sticky** re-attaches a ball on a genuine top-face paddle hit while `stickyEffect` is active
> [2845–2854](../html/index.html#L2845-L2854), capped to one attached ball at a time so multi-ball
> can't stack several on the paddle at once. `updatePaddle()`'s attached-ball tracking, previously
> hardcoded to `balls[0]`, now loops over every ball [2502–2506](../html/index.html#L2502-L2506) since
...
> `handleLaunchOrResume()` [1925–1935](../html/index.html#L1925-L1935) now calls `fireLaser()`
> [1958–1970](../html/index.html#L1958-L1970), which fires classic twin bolts from the paddle on a
> cooldown (`CONFIG.laser` [1198–1203](../html/index.html#L1198-L1203)). `updateLasers()`
> [2619–2640](../html/index.html#L2619-L2640) moves them and reuses `brickHit()` on impact — the same
> scoring/combo/difficulty path a ball hit goes through — and `drawLasers()`
> [3357–3370](../html/index.html#L3357-L3370) renders them. Releasing a sticky ball and firing both
> route through the same action-button entry point used everywhere else (mouse, touch, Space), via a
> new `launchAttachedBalls()` helper [1937–1950](../html/index.html#L1937-L1950) `launchBall()`
...
> [3132–3160](../html/index.html#L3132-L3160), called after every `applyPowerup()`
> [2598](../html/index.html#L2598) and once per frame [3471](../html/index.html#L3471). `state.widthEffect`/
...
> [3296](../html/index.html#L3296) already used for its colour swap.
...
> intact (`checkLevelClear()`'s `LEVELS.length - 1` win check [2506](../html/index.html#L2506), the HUD's
> `n/LEVELS.length` readout [2640](../html/index.html#L2640), and `level.of`'s `{n}/{total}` string all
...
> #31 — see `updateEffectBar()` [3132–3141](../html/index.html#L3132-L3141)) resizes only that
...
> `OVERLAY_BUTTON_IDS` lookup [2070–2079](../html/index.html#L2070-L2079) is built from
> `PHASE_OVERLAY`'s button entries (from `OVERLAY_PRIMARY_BTN`'s values at the time; #36 below folded
> that map into `PHASE_OVERLAY`), and `showOverlay()` [2101–2104](../html/index.html#L2101-L2104)
...
> [2048](../html/index.html#L2048) — `OVERLAY_PRIMARY_BTN` already had the matching
> `"overlay-start": "btn-start"` since #26 [2048](../html/index.html#L2048) — so boot
> [3484](../html/index.html#L3484) now calls `setPhase("start")` instead of `showOverlay(...)`
...
> **Fixed 2026-08-13.** `touchend`'s handler [1908–1924](../html/index.html#L1908-L1924) now only
...
> **Fixed 2026-08-13.** `PHASE_OVERLAY` [2047–2061](../html/index.html#L2047-L2061) is now the only
...
> [2070–2079](../html/index.html#L2070-L2079) (see #33) and `setPhase()`
> [2117–2125](../html/index.html#L2117-L2125) both derive what they need from `PHASE_OVERLAY` alone,
...
> existing overlap test — [2829–2841](../html/index.html#L2829-L2841). When the ball's start-of-frame
...
([3423](../html/index.html#L3423)). It never factors in `state.difficultyMult`
([1578](../html/index.html#L1578)), the mid-level ramp (up to `CONFIG.difficulty.max` = `1.6`,
[1212](../html/index.html#L1212)) that's multiplied into the same per-frame displacement at
[2813](../html/index.html#L2813):
...
"Niveau 1 / 10" now). `updateHud()` ([2640](../html/index.html#L2640)) overwrites it with the real
...
> `PHASE_OVERLAY` entry ([2060–2061](../html/index.html#L2060-L2061)) rather than bolting an input onto
> `overlay-victory`/`overlay-gameover` directly. `endGame()` ([2974–2983](../html/index.html#L2974-L2983))
...
> ([3012–3014](../html/index.html#L3012-L3014)) is true: strictly greater than 0, and either the board
> has room or the score beats its current lowest entry via `hallOfFameRank()`
> ([3000–3005](../html/index.html#L3000-L3005)) — a tie with the lowest entry does not bump it. The
> board is a capped, sorted `{name, score}` list under a new `neonbreak-hall-of-fame` key
> ([1446](../html/index.html#L1446)), round-tripped through `loadHallOfFame()`/`saveHallOfFame()`
> ([1478–1491](../html/index.html#L1478-L1491)) via the same guarded `storageGet`/`storageSet` pair #2
...
> [1280–1283](../html/index.html#L1280-L1283)), and falls back to a translated `"???"` placeholder when
> empty (`submitHallOfFameName()`, [3034–3051](../html/index.html#L3034-L3051)). `renderHallOfFame()`
> ([3058–3083](../html/index.html#L3058-L3083)) rebuilds the board through `innerHTML` rather than
...
> ([1809–1812](../html/index.html#L1809-L1812)), so Space still reaches the name field instead of being
> hijacked for launch/laser; Enter submits directly from the field
> ([1859–1861](../html/index.html#L1859-L1861)) since nothing else in this file uses a `<form>`.
>
...
Feature request: when a run ends (`endGame()`, [2974](../html/index.html#L2974)) with a score that
...
`loadBest()`/`saveBest()` ([1461–1462](../html/index.html#L1461-L1462)) under `BEST_KEY`
([1443](../html/index.html#L1443)), both guarded by `storageGet`/`storageSet`
([1452–1460](../html/index.html#L1452-L1460)) per #2. This replaces "a number" with "a list":
...
**Where it hooks in:** both `endGame(true)` and `endGame(false)` ([2974](../html/index.html#L2974)) —
...
(`PHASE_OVERLAY` [2047–2061](../html/index.html#L2047-L2061), `overlay-victory`/`overlay-gameover`
...
- *i18n.* Every new string (the name-entry prompt, its input placeholder, the hall-of-fame title, an
  empty-list message) needs a key in both `STRINGS.fr` and `STRINGS.en` ([1295](../html/index.html#L1295))
...
- *Keyboard/focus.* The name-entry overlay's input should get focus the way every other overlay's
  primary button does today (`showOverlay()` [2080](../html/index.html#L2080), #26), and
...
> ([2224–2238](../html/index.html#L2224-L2238)) sets `state.returnPhase = "start"` and calls
...
> (`"start"` / `"victory"` / `"gameover"`, [1588–1593](../html/index.html#L1588-L1593)): `endGame()`
> ([2982](../html/index.html#L2982)) sets it to `won ? "victory" : "gameover"` before the post-game
> detour exactly as `pendingWon` did, and the continue button
> ([2265–2267](../html/index.html#L2265-L2267)) just does `setPhase(state.returnPhase)` — one field
...
([1446](../html/index.html#L1446)), so two players never see each other's scores, and the same person
...
interpolated values ([3079–3080](../html/index.html#L3079-L3080)), so XSS is handled, but a public
...
> ([1242–1254](../html/index.html#L1242-L1254)) and driven from three timers on `state`
> ([1604–1609](../html/index.html#L1604-L1609)): a camera shake on an explosion
> ([2683–2684](../html/index.html#L2683-L2684)) and on a lost ball
> ([2910](../html/index.html#L2910)), 55 ms of frozen simulation with the blast, and a paddle squash
> on every steered bounce ([2864](../html/index.html#L2864)). The whole layer lives in one block —
> [1748–1801](../html/index.html#L1748-L1801).
>
> **It is presentation, and the boundary is enforced rather than described.** The shake is a
> `ctx.translate` around the whole scene in `draw()` ([3402–3407](../html/index.html#L3402-L3407)),
> so nothing the game simulates moves because of it, and the squash is applied to the paddle's drawn
> rectangle only ([3300–3310](../html/index.html#L3300-L3310)) — `state.paddle.h` still governs
...
>
> **Hit-stop is set, never accumulated** ([1767–1772](../html/index.html#L1767-L1772)). Summing it
...
> ([1688–1690](../html/index.html#L1688-L1690)), so no path leaves the simulation frozen.
>
> `drawBackground()` now bleeds past the play area by the largest possible offset
> ([3202–3210](../html/index.html#L3202-L3210)); an exactly sized fill leaves a strip of the
...
> ([2314–2344](../html/index.html#L2314-L2344)), the single primitive everything audible is built
...
> `MUSIC_KEYS` ([2350–2353](../html/index.html#L2350-L2353)) pitch the music, the brick voices and
...
>
> **A voice per brick type.** `BRICK_VOICE` ([2368–2379](../html/index.html#L2368-L2379)) gives each
...
> changes what a hit sounds like. `brickTone()` ([2383–2391](../html/index.html#L2383-L2391))
> replaces the four hand-tuned `beep()` calls that used to be scattered through `brickHit()`
> ([2728–2765](../html/index.html#L2728-L2765)).
>
> **A ladder for streaks.** `ladderSemi()` ([2360–2363](../html/index.html#L2360-L2363)) climbs a
...
> raised ([2750](../html/index.html#L2750)) so a hit sounds on the rung it just earned.
>
> **The bed.** Four voices over a 16-step bar ([2427–2437](../html/index.html#L2427-L2437)), queued
> by `updateMusic()` ([2472–2489](../html/index.html#L2472-L2489)) from `frame()`
> ([3435](../html/index.html#L3435)) and tuned in `CONFIG.music`
> ([1231–1237](../html/index.html#L1231-L1237)). Three things about it are deliberate:
>
...
> - **A stall resyncs rather than catching up** ([2482](../html/index.html#L2482)). A backgrounded
...
>   ([2459–2466](../html/index.html#L2459-L2466)) rises instantly to whatever `voiceCombo` tier the
...
> Mute needed no change: `audioCtx()` ([2295–2307](../html/index.html#L2295-L2307)) returns null
...
> ([1666–1667](../html/index.html#L1666-L1667)) rather than per frame.
>
...
> ([3182–3199](../html/index.html#L3182-L3199)), all derived from one number — `state.bgScroll`,
> seconds of real time accumulated in `frame()` ([3440](../html/index.html#L3440)). Nearer layers
...
> ([3168–3180](../html/index.html#L3168-L3180)) — `createLinearGradient` allocates.
>
...
> `buildLevel()` ([1640](../html/index.html#L1640)) and `resetPaddleAndBall()`
> ([1677](../html/index.html#L1677)), and neither can tell the difference.
> `CONFIG.progression.totalLevels` ([1175–1182](../html/index.html#L1175-L1182)) replaced
> `LEVELS.length` in `checkLevelClear()` ([2960](../html/index.html#L2960)), `renderDynamicText()`
> ([2008](../html/index.html#L2008)) and `updateHud()` ([3104](../html/index.html#L3104)), and the
...
> in `brickHit()` ([2744](../html/index.html#L2744)).
>
> **Relief:** three lives across 100 levels is not survivable, so clearing every 10th level hands one
> back, capped at `state.maxLives` ([2963–2969](../html/index.html#L2963-L2969)). Awarded on the way
...
> ([1838–1843](../html/index.html#L1838-L1843)), reading the `state.keys` set the paddle already
...
> ([1817–1820](../html/index.html#L1817-L1820)), deliberately narrower than the existing
...
> ([2068](../html/index.html#L2068)), not a modal bolted on beside the phase machine. That is the
...
> the handler gets a look in ([1828–1833](../html/index.html#L1828-L1833)).
>
> `openLevelJump()`/`cancelLevelJump()`/`submitLevelJump()`
> ([1993–2031](../html/index.html#L1993-L2031)) are the whole of it. Cancelling restores the phase
...
> `newGame()`, and checked in both `endGame()` ([2981](../html/index.html#L2981)) and
> `maybeSaveBest()` ([2950](../html/index.html#L2950)). The world board can never be reset (#67) and
...
> jumping mid-run keeps both. `RUN_PHASES` ([1991](../html/index.html#L1991)) is the distinction.
>
> **One bug this surfaced in existing code.** `showOverlay()` blurred a leftover focused control only
> when it was a `BUTTON` ([2095–2098](../html/index.html#L2095-L2098)). That was harmless while
...
>
> **The beat.** `loseLife()` ([2908–2927](../html/index.html#L2908-L2927)) no longer transitions; it
> sets `state.lifeLost = {remaining, ended}` and moves to a new `lifelost` phase
> ([2053](../html/index.html#L2053)), which `frame()` spends a frame at a time
> ([3452–3455](../html/index.html#L3452-L3455)) before calling `finishLifeLost()`
> ([2932–2941](../html/index.html#L2932-L2941)) — the other half of the old function, serving again or
> ending the run. `CONFIG.impact.lifeLostBeat` is 0.7 s
> ([1257](../html/index.html#L1257)). Making it a phase rather than a counter checked beside the
...
>
> **The burst** is two calls ([2918–2919](../html/index.html#L2918-L2919)): white for the ball coming
...
> ball's last x ([2898](../html/index.html#L2898)), because by the time `loseLife()` runs the ball is
...
>
> **The sting** ([2398–2417](../html/index.html#L2398-L2417)) is four notes falling through the
...
> ([1250–1257](../html/index.html#L1250-L1257)): it is pacing, not motion, and #58's rule is that the
...
> ([1322](../html/index.html#L1322), [1380](../html/index.html#L1380)), which says both halves of it:
> the run is out of the hall of fame, and playing again gives an eligible one.
>
> It is written in `renderDynamicText()` ([2152–2154](../html/index.html#L2152-L2154)) rather than in
...
([2981](../html/index.html#L2981)) and `maybeSaveBest()` ([2950](../html/index.html#L2950)). The
...
>
> The three entry points now share `viewHallOfFame(from)` ([2231–2235](../html/index.html#L2231-L2235))
...
> underneath it (#67).
>
> **Restart stays the primary control.** `PHASE_OVERLAY` ([2062–2063](../html/index.html#L2062-L2063))
...
>
> **`start.viewHof` became `hof.view`** ([1307](../html/index.html#L1307),
> [1365](../html/index.html#L1365)) across both tables and all three markup sites. The text was
