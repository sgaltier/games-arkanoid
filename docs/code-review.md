...
**Status:** #1–#10 fixed, 22 items open. What shipped and when is tracked in
...
> [:1098–1101](../arkanoid.html#L1098-L1101), and the call site at [:1451](../arkanoid.html#L1451).
...
> [:1207](../arkanoid.html#L1207). A paddle collision is only resolved as a top-face bounce — steering
> by offset and snapping onto the top — when `prevY` was already above the paddle top
> [:1219–1227](../arkanoid.html#L1219-L1227); otherwise it resolves as a side hit that reflects only
> the horizontal component and repositions the ball beside the paddle, the same treatment a brick's
> side face gets [:1228–1235](../arkanoid.html#L1228-L1235). Paddle-velocity spin was left for a
...
> [:1175–1184](../arkanoid.html#L1175-L1184) — the smaller of the two axis overlaps, i.e. how shallow
> the intrusion is — and resolves against whichever brick has the smallest penetration
> [:1239–1257](../arkanoid.html#L1239-L1257). Array order no longer has any say in which face gets hit.
...
`updateDrops` tests `± 8` [:1125–1126](../arkanoid.html#L1125-L1126); `drawDrops` renders `arc(0,0,10,…)`
[:1408](../arkanoid.html#L1408). Power-ups visually clip the paddle without being collected.

### 12. Multi-ball can spawn balls aimed straight down (S)
[:1108](../arkanoid.html#L1108) — the clone angle is `atan2(base.dy, base.dx) ± 0.6`. If the source ball
...
`endGame` [:1296–1300](../arkanoid.html#L1296-L1300) is the only caller of `saveBest()`. Closing the
...
[:1411](../arkanoid.html#L1411), inside the `drawDrops` loop. This forces a synchronous style
...
Called unconditionally at [:1456](../arkanoid.html#L1456) in addition to the event-driven calls in
...
[:1281–1285](../arkanoid.html#L1281-L1285) runs `.some()` over up to 80 bricks each frame. Cheap in
...
[:913](../arkanoid.html#L913), `checkLevelClear` [:1289](../arkanoid.html#L1289), and `endGame`
[:1304](../arkanoid.html#L1304) each assign `state.phase` *and* call `showOverlay` directly. That's the
...
- The `updateHud(); drawBackground(); drawBricks(); drawPaddle();` block at
  [:1465–1469](../arkanoid.html#L1465-L1469) is redundant; the rAF loop paints the same frame ~16 ms later.
- `updateBalls(dt, now)` [:1199](../arkanoid.html#L1199) never uses `now`.
...
Magic numbers are scattered through the file: drop fall speed `130` [:1123](../arkanoid.html#L1123), particle
gravity `260` [:1142](../arkanoid.html#L1142), effect durations `10`/`8` seconds [:1098–1101](../arkanoid.html#L1098-L1101),
multipliers `1.6`/`0.6`/`0.7`/`1.4`, ball-cap `5`, paddle bounce spread `1.05` [:1224](../arkanoid.html#L1224).
...
The paddle changes colour for width effects [:1367](../arkanoid.html#L1367), but there is no indication
